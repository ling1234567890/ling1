...
//				console.log(hourLeft)
	...
	//我的账户鼠标移入效果
	 function show(){
	 	...
	 	$sidebar.each(function(idx,ele){
	 		$(ele).hover(function(){
	 			$tab_tip.eq(idx).animate({right:'38px',opacity: 1});
	 		});
	 	});
	 	$tab.each(function(idx,ele){
	 		$(ele).mouseleave(function(){
	 			$tab_tip.eq(idx).animate({right:'70px',opacity: 0,},200);
	 		})
	...
	function sidecart_cont(e){
		$('.sidecart-text').on('click',function(){
			$('.sidecart-cont').css({display:' block'})
		});
//		e.preventDefault();
//		e.stopPropagation();
		$('.icon-remove-circle').on('click',function(){
			$('.sidecart-cont').css({display:' none'})
			console.log(111)
		});
